  ...
  const checkServices = async () => {
    ...
    if (!supabaseUrl || supabaseUrl === "https://placeholder.supabase.co") {
      results.push({
        ...
        status: "error",
        message:
          "Missing or placeholder value. Set NEXT_PUBLIC_SUPABASE_URL in .env.local",
      });
    } else {
      results.push({
        name: "Supabase URL",
        status: "success",
        message: "Configured (URL not shown)",
      });
    }
    ...
    if (!supabaseKey || supabaseKey === "placeholder") {
      results.push({
        ...
        status: "error",
        message:
          "Missing or placeholder value. Set NEXT_PUBLIC_SUPABASE_ANON_KEY in .env.local",
      });
    } else {
      results.push({
        name: "Supabase Anon Key",
        status: "success",
        message: "Configured (key value not shown)",
      });
    }
    ...
    if (!maptilerKey) {
      results.push({
        ...
        status: "warning",
        message:
          "Not configured. Maps may not load. Set NEXT_PUBLIC_MAPTILER_API_KEY",
      });
    } else {
      results.push({
        name: "MapTiler API Key",
      ...
      <div className="max-w-3xl mx-auto">
        <div className="bg-white rounded-2xl shadow-lg p-8 mb-6">
          <div className="text-center">
            ...
            <p className="text-gray-600">Environment configuration status</p>
          </div>
        </div>
            ...
            <div>
              ...
              <p className="text-sm text-gray-600">
                {allGood
                  ...
                  : hasErrors
                    ? "Please fix the errors below to use the application"
          ...
          {services.map((service) => (
            ...
            >
              <div className="flex items-start gap-4">
                <div className="flex-shrink-0 mt-1">{getIcon(service.status)}</div>
                <div className="flex-1 min-w-0">
                  <h3 className="text-lg font-semibold text-gray-900 mb-1">
                    {service.name}
                  </h3>
                  <p className="text-sm text-gray-600 break-all">{service.message}</p>
                </div>
              </div>
            </div>
          ))}
        ...
        {hasErrors && (
          <div className="mt-6 bg-blue-50 border-2 border-blue-200 rounded-2xl p-6">
            <h3 className="text-lg font-bold text-blue-900 mb-3">📋 How to Fix</h3>
            <ol className="list-decimal list-inside space-y-2 text-sm text-blue-800">
              <li>
                Create a file named{" "}
                <code className="bg-blue-100 px-2 py-1 rounded">.env.local</code> in
                the project root
              </li>
